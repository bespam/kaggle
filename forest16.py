...
def train():
    ...
    train_data = np.insert(train_data, 11, values = 0,axis = 1)
    train_data = np.insert(train_data, 12, values = 0,axis = 1)
    ...
    #normaliza class
    test_data[test_data[0::,1] == '1',1] = 1.0
    test_data[test_data[0::,1] == '2',1] = 2.0
    test_data[test_data[0::,1] == '3',1] = 3.0
    
    #Male = 1, female = 0:
    test_data[test_data[0::,3]=='male',3] = 1.0
    test_data[test_data[0::,3]=='female',3] = 0.0
    #ebark c=0, s=1, q=2
    test_data[test_data[0::,10] =='C',10] = 1.0 #Note this is not ideal, in more complex 3 is not 3 tmes better than 1 than 2 is 2 times better than 1
    test_data[test_data[0::,10] =='S',10] = 2.0
    test_data[test_data[0::,10] =='Q',10] = 3.0



    #Create 2 more columns from name: family name, title
    header.extend(["Family name","Title"])
    ...
    for i in range(test_data.shape[0]):
        ...
        if val[0].strip() not in last_names.keys():
            last_names[val[0].strip()] = name_count
            name_count += 1
        test_data[i,12] = titles[val[1].strip()]# /3.0  
        #ticket
        if test_data[i,7] not in tickets.keys():
            tickets[test_data[i,7]] = ticket_count
            ticket_count += 1
 
    for i in range(test_data.shape[0]):
        name = test_data[i,2]
        val = name.replace(".",",").split(",")     
        test_data[i,11] = last_names[val[0].strip()]#/float(len(last_names.keys())-1)
        test_data[i,7] = tickets[test_data[i,7]]#/float(len(tickets.keys())-1) 
    ...
    test_age_exist = test_data[(test_data[:,4] != ''),:]
    test_age_exist = test_age_exist[:,[4,3,5,6,11,12]]
    test_age_missing = test_data[(test_data[:,4] == ''),:]
    test_age_missing = test_age_missing[:,[3,5,6,11,12]]
    print 'Training missing age values'
    ...
    test_data[(test_data[:,4] == ''),4] = output
    ...
    for i in range(test_data.shape[0]):
        if test_data[i,10] == "":
            test_data[i,10] = np.round(np.mean(test_data[(test_data[0::,10]\
                                                       != '') * (test_data[0::,0] == test_data[i,0]),10].astype(np.float)))
    ...
    """
    #dum = [header.pop(i) for i in [1,6,8,10,11]]
    ...
    open_file_object = csv.writer(open("csv/forest16.csv", "wb"))
